...
export async function buildTcTokenFromJid({
	...
	try {
		const storageJid = jid // pode vir em lid ou jid
		const tcTokenData = await authState.keys.get('contacts-tc-token', [storageJid])
 ...
 */
export async function storeTcTokensFromIqResult({ result, fallbackJid, keys, onNewJidStored }: StoreTcTokensParams) {
	...
	for (const tokenNode of tokenNodes) {
		...
		const storageJid = jidNormalizedUser(tokenNode.attrs.jid || fallbackJid)
		const existingTcData = await keys.get('contacts-tc-token', [storageJid])
